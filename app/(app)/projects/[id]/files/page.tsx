...
export default async function FilesPage({
  ...
  return (
    <WorkspaceContainer>
      <SectionHeading
        title="Files"
        description="Inspect persisted project files. AI, validation, and runtime read from this saved file set."
      />
      ...
      {error ? (
        ...
        <Empty className="border border-dashed border-border bg-card/40">
          <EmptyHeader>
            ...
            <EmptyDescription>
              Files appear here only after a generation or repair run passes
              validation and persistence. Failed task diagnostics stay on the AI
              tab.
            </EmptyDescription>
          </EmptyHeader>
        </Empty>
          ...
          <section className="min-w-0 overflow-hidden rounded-lg border border-border bg-card">
            {selectedFile ? (
              ...
              <div className="flex min-h-64 items-center justify-center p-6 text-sm text-muted-foreground">
                Select a saved file to inspect its persisted contents.
              </div>
            )}
          </section>
        </div>
